...
class ParityLSTM(torch.nn.Module) :
    ...
    def forward(self, x, s):
        # h0 = torch.zeros(self.num_layers,x.size(0),self.hidden_dim).to(dev)
        # c0 = torch.zeros(self.num_layers,x.size(0),self.hidden_dim).to(dev)
        x = x.unsqueeze(-1)
...
def runParityExperiment(model, max_train_length):
    ...
    accuracy = torch.FloatTensor(accuracy).numpy()
    plt.figure()
